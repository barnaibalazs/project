...
ggplot(data = superstore,aes(x=Sales,y=Profit)) +
  labs(title="A profit alakulása az eladások volumenének függvényében")+
  geom_point()
```
...
ggplot(data=superstore,aes(x=Category)) +
  labs(title="Termékkategóriák gyakorisági eloszlása")+
...
ggplot(data=superstore,aes(x=Sales,y=Profit)) +
  ...
  labs(title="A profit ingadozása az eladási érték függvényében")+
  theme_minimal()
```
...
ggplot(data = superstore,aes(x=Sales,y=Profit)) +
  ...
  labs(title="Az értékesítés és a nyereség általános trendje")+
  theme_minimal()
```
